...
print(torch.cuda.current_device())  # Should return the GPU device ID
print(torch.cuda.device(0))  # Should print the GPU device details
...
from transweather_model import TransweatherTNT as Transweather

plt.switch_backend('agg')

# --- Parse hyper-parameters  --- #
parser = argparse.ArgumentParser(description='Hyper-parameters for network')
parser.add_argument('-learning_rate', help='Set the learning rate', default=0.01, type=float)
...
if not os.path.exists(f'./{exp_name}/'):
    os.mkdir(f'./{exp_name}/')
try:
    net.load_state_dict(torch.load(f'./{exp_name}/best'))
    print('--- weight loaded ---')
except:
...
# --- Load training data and validation/test data --- #
lbl_train_data_loader = DataLoader(TrainData(crop_size, train_data_dir, labeled_name), batch_size=train_batch_size, shuffle=True, num_workers=0, pin_memory=True)
val_data_loader1 = DataLoader(ValData(val_data_dir, val_filename1), batch_size=val_batch_size, shuffle=False, num_workers=0, pin_memory=True)
